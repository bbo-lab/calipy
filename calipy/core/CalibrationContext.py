...
class CalibrationContext(BaseContext):
    ...
    def load_result(self, url):
        with open(url, "rb") as file:
            ...
            print(f"Current software version: {VERSION}, Calipy file version: {temp.get('version', None)}.")
    ...
    def load_result_npy(self, url):
        ...
        if isinstance(temp_npy['info']['opts']['free_vars'], list):
            # In the later version of calibcam, the free_vars is a list with free_vars for each camera
            camera_model_id = "opencv-omnidir" if temp_npy['info']['opts']['free_vars'][0]['xi'] else "opencv-pinhole"
        else:
            camera_model_id = "opencv-omnidir" if temp_npy['info']['opts']['free_vars']['xi'] else "opencv-pinhole"
        ...
        for index, detor in enumerate(self.detectors):
            if calibcam_det_id in detor.ID:
                self.detector_index = index
                break
        detector = self.get_current_detector()
