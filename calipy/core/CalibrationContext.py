...
class CalibrationContext(BaseContext):
    ...
    @staticmethod
    def set_current_detector_parameter(name, value):
    ...
    def calibrate_cameras(self, progress):
        ...
        for cam in self.get_cameras():
            ...
            for batch_size in range(total // 4, total, (total // 4) + 1):
                ...
                try:
                    calibration = model.calibrate_camera(sizes[0], object_pts[:batch_size], image_pts[:batch_size],
                                                         calibration)
                except Exception as e:
                    print(
                        "Batch Calibration with {:d} / {:d} detections failed: {:s}".format(batch_size, total, str(e)))
    ...
    def prepare_syscal_frames(self, cam_frame_bool, source_maps, estimations_all):
        ...
        # Identify the poses (corresponding frames) that were PROPERLY captured in more than one camera for system
        # calibration. A PROPER capture is when number of detected features >= min_det_feats
    ...
    def prepare_syscal(self, frame_bool: np.ndarray, square_ids: dict, image_pts: dict, estimations: dict,
                       parameters: dict, detections: dict, estimations_old: dict):
        """ Update the dictionaries: square_ids, image_pts, and estimations"""
    ...
    def calibrate_system(self, progress):
        ...
        num_orig_frames = [rec.get_length() for rec in self.recordings.values()]
        ...
        # Extract square_ids, image points, intrinsic and extrinsic parameters
        square_ids = {}
        image_pts = {}
        estimations_new = {}
        calibrations_new = {}
        ...
        for cam_idx, (cam) in enumerate(self.get_cameras()):
            ...
            sq_ids, img_pts, estimations = ({} for _ in range(3))
            ...
            for src_id in source_ids:
                ...
                self.prepare_syscal(frame_bool, sq_ids, img_pts, estimations,
        ...
        progress.setLabelText(
            "System calibration in progress: {:d} cameras and {:d} poses".format(num_cams, np.int(sum(frame_bool))))
        if progress.wasCanceled():
            return

        # Run system calibration
        syscal = model.calibrate_system(sizes[0], square_ids, image_pts, estimations_new, calibrations_new, flags,
                                        syscal)
