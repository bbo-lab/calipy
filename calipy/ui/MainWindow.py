...
class MainWindow(QMainWindow):
    ...
    def open(self, file, storage=None):
        ...
        if file.endswith(".npy"):
            # Generated by Calibcam
            temp_npy = np.load(file, allow_pickle=True).item()
            rec_files = temp_npy['rec_file_names']
            rec_pipelines = temp_npy.get('rec_pipelines', [None] * len(rec_files))
            self.context.add_session()
            for idx, rec_file in enumerate(rec_files):
                if Path(rec_file).exists():
                    self.context.add_camera(str(idx))
                    self.context.add_recording(str(idx), rec_file, rec_pipelines[idx])
            ...
            if "anatomical_setup" in config_dict:
                videos_dict = config_dict['anatomical_setup'].get('cameras', {})
                if videos_dict:
                    self.context.add_session()
